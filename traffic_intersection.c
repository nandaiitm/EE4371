...
int main(int argc, char **argv)
{
    if(argc != 2)
    {
        printf("Error:  Run the program in ./program <filename.dat> format\n");
        exit(1);
    }
    FILE *file;
    file = fopen(argv[1],"r");
    ...
    fscanf(file,"%1d",&num_vertices); // Reads the first line in file inorder to determine the number of vertices
    for(int i = 0; i <= num_vertices; i++) // Obtains the names of vertices from the file and stores them in an array
    {  
    ...
    for(int i = 0; i <= num_vertices; i++) // Obtains the value of edges between vertices and stores in a 2-Dimensional array
    {
        fgets(row,30,file);
        char *pos;
        if((pos=strchr(vertice_names[i],'\n')) != NULL)
    ...
    initialize_colored();  // initialize colored and extra boolean arrays
    initialize_extra();
