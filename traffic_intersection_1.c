...
int n;
int incid_mat[30][30]; // Stores the edges between vertices in an square matrix
...
void initialize_colored() // Initializes the array to indicate status of colored vertices
{
    for(int i = 0; i <n; i++){
        is_colored[i] = 0;
    }
...
void initialize_extra()  // Initializes the array for extra turns
{
    for(int i = 0; i < n; i++){
        extra_turns[i] = 1;
    }
...
int isAllColoured() // Determines if all the vertices are colored or not
{
    int flag=1;
    for(int i=0; i<n; i++)
    {
...
void greedy()  // Greedy algorithm to color the vertices of the same group
{
    int found;
...
int main( )
{
  ...
  */
  while(!isAllColoured()) // Iterates until greedy colors all the vertices
  {
      greedy(); // colors vertices of the same group
      printf("%d\t",color);
      for(int i = 0; i < n; i++)
      {
